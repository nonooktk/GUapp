...
/**
 * 商品詳細 DS-SCR-003（設計仕様書 6.5・U-05・REQ-FR-1203、テスト ST-F005-01〜03・AT-03）。
 * 画像（複数）・名称・税込価格・色/サイズ/数量（PurchasePanel）・説明・素材・関連商品。
...
export default async function ProductDetailPage({ params }: PageProps<"/products/[id]">) {
  ...
  return (
    <article className="space-y-10 pb-28 sm:pb-0">
      <nav aria-label="パンくず" className="text-sm text-gray-600">
        <ol className="flex flex-wrap items-center gap-1">
          ...
          <li aria-hidden="true">›</li>
          <li aria-current="page" className="truncate font-bold text-gray-900">
            {product.name}
          </li>
        </ol>
      </nav>
      ...
      <div className="grid gap-6 md:grid-cols-2">
        ...
        <section aria-label="商品画像">
          <ul className="flex snap-x snap-mandatory gap-2 overflow-x-auto rounded-lg">
            {images.map((path, i) => (
              <li key={`${path}-${i}`} className="w-full shrink-0 snap-center">
                ...
                <img
                  ...
                  className="aspect-[3/4] w-full rounded-lg bg-gray-100 object-cover"
                />
              </li>
            ))}
          </ul>
          {images.length > 1 && (
            <p className="mt-1 text-center text-xs text-gray-500">横にスワイプして {images.length} 枚の画像を見る</p>
          )}
        </section>
      ...
      <section aria-labelledby="desc-heading" className="space-y-4 text-sm leading-relaxed">
        <h2 id="desc-heading" className="text-lg font-bold">
          商品説明
        </h2>
        ...
        <ul className="flex flex-wrap gap-2">
          {product.sizes.map((s) => (
            <li key={s} className="rounded border border-gray-300 px-2 py-0.5">
              {s}
            </li>
          ))}
        </ul>
      ...
      {product.related.length > 0 && (
        <section aria-labelledby="related-heading">
          ...
          <ul className="grid grid-cols-2 gap-x-3 gap-y-6 sm:grid-cols-4" aria-label="関連商品">
