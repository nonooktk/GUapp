...
MD = r'C:\Users\m-oya\Desktop\GUapp\docs\04_テスト設計書\GU_ECsite_テスト設計書_P1_draft-v1.md'
...
OUT = r'C:\Users\m-oya\Desktop\DocsMaker\成果物\GU_ECsite_テスト設計書_P1_draft-v1_20260907.docx'
FONT = 'メイリオ'
MONO = 'ＭＳ ゴシック'
HEADER_FILL = 'F2F2F2'
...
r = hdr.add_run('GU ECサイト テスト設計書（P1 本編）'); r.font.name = FONT; r.font.size = Pt(8); r.font.italic = True
...
def add_image(idx):
    ...
    doc.add_picture(path, width=TEXT_W)
...
para('テスト設計書（P1 本編）', size=26, bold=True, align=WD_ALIGN_PARAGRAPH.CENTER, space_after=6)
...
while i < n:
    ...
    if ln.startswith('```'):
        ...
        if lang == 'mermaid':
            mermaid_idx += 1; add_image(mermaid_idx)
        else:
            code_block(block)
        i = j + 1; continue
    if ln.startswith('|'):
