...
| draft-v1（デモの個人情報対策を追加） | 2026-09-25 | 統括が試しに購入した際、本物の氏名・住所・メール・電話を入力してしまった事象を受け、デモで本物の個人情報を入力・保存させないための対策を追加。新規 DS-DEC-47（dev サーバーを 127.0.0.1 限定・注文手続きの初期値にダミー値と注意書きを常時表示・講義サーバーに本物の個人情報を入れない）を 9.1 に追加し、6.6 に DS-SCR-005 注文手続きの差分（本編 6.7 の差分）を新設。ADR-0002 の変更履歴にも同日付で追記 | ポリゴン／キャタピー（Claude） | Oya |
...
| 判断 | DS-DEC-33〜47 | 本書 9.1 に一覧（2026-09-24 差し戻しで 46 を追加、2026-09-25 に 47 を追加） |
...
### 6.6 DS-SCR-005 注文手続き（デモの個人情報対策。本編 6.7 の差分。2026-09-25 追加）

お届け先の見出し直後（各入力欄より上）に、`role="note"` の案内ボックスで「講義演習用のデモサイトです。入力欄にはダミーの値が入っています。実際の氏名・住所・電話番号・メールアドレスは入力しないでください。」を常時表示する。375px でも折り返して読める 1 カラムの文章で、色だけで意味を伝えない（背景色に頼らずテキストで明示する）。あわせて、氏名・郵便番号・都道府県・市区町村・番地・建物名・電話番号・メールアドレスの各欄は、`sessionStorage`（`guapp.checkout`）に確認画面から戻った復元値が無いときだけ、架空のダミー値（氏名「デモ 太郎」、住所「東京都 千代田区 デモ町1-2-3 サンプルビル101」、電話「090-0000-0000」、メール `demo@example.com`）を初期値にする。復元値がある場合はダミー値で上書きしない。ダミー値は入力形式（4.4）をそのまま満たし、送信前のクライアント検査・サーバー側の `OrderCreateIn`（DS-API-022）検証のどちらも素通しで通る（DS-DEC-47）。
...
| 47 | **（2026-09-25 追加）** デモで本物の個人情報を入力・保存させないための対策を 3 つ入れる。①`apps/web` の `pnpm dev` は `next dev -H 127.0.0.1` とし、この Mac（127.0.0.1）からだけ開けるようにする（`start`＝App Service の本番起動は変えない）。②注文手続き（DS-SCR-005）の配送先入力欄は、`sessionStorage`（`guapp.checkout`）に復元値が無いときだけ架空のダミー値（氏名・郵便番号・住所・電話番号・メールアドレス。`apps/web/lib/demo-defaults.ts`）を初期値にし、配送先の直前に「講義演習用のデモです。実際の氏名・住所・電話番号・メールアドレスは入力しないでください」の趣旨の注意書きを常時表示する。③講義の Azure MySQL（ADR-0002）に本物の個人情報を入れない運用をドキュメント（ADR-0002 変更履歴・`scripts/azure-db/README.md`）に明記する。**この初期値と注意書きは本番（P2-d で Azure App Service に公開）でも環境で出し分けず常時表示する**（本サイト自体が講義演習用のデモであり、公開後も個人情報を入力させたくない前提は変わらないため） | 統括が試しに購入した際、開発サーバーが全インターフェースで待ち受けていたため同じ Wi-Fi の他端末から開けた／講義 Azure MySQL の管理者アカウントは講義で共有され自分のスキーマも読める、という 2 点に気付き、本物の氏名・住所・メール・電話を入力してしまった事象への再発防止 | ダミー値・注意書きを開発環境だけに出す（環境変数で出し分ける）実装も検討したが、本番でもデモである以上は同じ対策が必要であり、環境分岐を増やすと「本番だけダミー値を消し忘れる」事故の余地が生まれるため不採用 |

### 9.2 P2 事前レビュー観点（8 問）記入表

| 観点 # | 該当 | 確認方法 | 結果・備考 |
| --- | --- | --- | --- |
| 1 数字と列挙の一致 | あり | 本書中の「N 件」「N 表」等の数字を、対応する列挙・表の行数と突き合わせ | 5.2 の seed 初期データ「8 件」は表の行数 8 と一致。4.3 の「5 種固定」は表の行数 5 と一致。9.1 の DS-DEC は 33〜47 の連番で 15 件、本表（9.2）は事前レビュー観点の 8 問固定で一致（2026-09-24 差し戻しで DS-DEC-46 を追加し 13→14 件、2026-09-25 に DS-DEC-47 を追加し 14→15 件に更新） |
